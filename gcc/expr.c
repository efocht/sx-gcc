...
emit_move_multi_word (enum machine_mode mode, rtx x, rtx y)
{
  ...
  if (GET_MODE_SIZE (mode) < UNITS_PER_WORD) {
	  fprintf(stderr, "%s: mode=%s  x = ", __FUNCTION__,
		  GET_MODE_NAME(mode));
	  print_inline_rtx(stderr, x, 0);
	  fprintf(stderr, " y = ");
	  print_inline_rtx(stderr, y, 0);
	  fprintf(stderr, "\n size=%d UnitePerWord=%d",
...
emit_move_insn (rtx x, rtx y)
{
  ...
  if (!(mode != BLKmode &&
	(GET_MODE (y) == mode || GET_MODE (y) == VOIDmode))) {
	  fprintf(stderr, "%s: mode=%s y=", __FUNCTION__, GET_MODE_NAME(mode));
	  print_inline_rtx(stderr, y, 0); fprintf(stderr, "\n");
  }
