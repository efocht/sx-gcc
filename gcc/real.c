...
/* The NEC SX quad precision format.  */

static void encode_sx_quad (const struct real_format *fmt,
            long *, const REAL_VALUE_TYPE *);
static void decode_sx_quad (const struct real_format *,
            REAL_VALUE_TYPE *, const long *);

static void
encode_sx_quad (const struct real_format *fmt, long *buf,
      const REAL_VALUE_TYPE *r)
{ 
  unsigned long image3, image2, image1, image0, exp;
  bool denormal = (r->sig[SIGSZ-1] & SIG_MSB) == 0;
  ...
  image3 = r->sign << (HOST_BITS_PER_LONG - 1);
  image2 = 0;
  image1 = 0;
  image0 = 0;
  
  rshift_significand (&u, r, SIGNIFICAND_BITS - 105);
      ...
      if (fmt->has_nans)
        {
          ...
          if (r->canonical)
            ...
            {
              image0 = u.sig[0];
              image1 = u.sig[1] & 0x000fffff;
              image2 = ((u.sig[2] & 0x000fffff) << 12) | 
                ((u.sig[1] & 0xfff00000) >> 20);
              image3 |= (((u.sig[3] & 0x000000ff) << 12) | 
                ((u.sig[2] & 0xfff00000) >> 20));
            }
          else
            {
              /* TODO: currently we only support floating point number 
                 generation for hosts with 32-bit longs. The support for 
                 hosts with 64 bit longs is yet to be implemented*/
              image0 = 0xffffffff;
              image1 = 0xffffffff;
              image2 = 0xffffffff;
              image3 = 0xffffffff;
...
static void
decode_sx_quad (const struct real_format *fmt, REAL_VALUE_TYPE *r,
      const long *buf)
{
  unsigned long image3, image2, image1, image0;
  bool sign;
      ...
      if ((image3 | image2 | image1 | image0) && fmt->has_denorm)
        {
          ...
          SET_REAL_EXP (r, -1022 + (SIGNIFICAND_BITS - 104));
          if (HOST_BITS_PER_LONG == 32)
            {
              r->sig[0] = image0;
              r->sig[1] = image1 & 0x000fffff;
              r->sig[1] |= ((image2 << 20) & 0xfff00000);
              r->sig[2] = (image2 >> 12) & 0x000fffff;
              r->sig[2] |= (image3 << 20) & 0xfff00000;
              r->sig[3] = (image3 >> 12) & 0x000000ff;
            }
          else
            {
              /* TODO: currently we only support floating point number 
               generation for hosts with 32-bit longs. The support for 
               hosts with 64 bit longs is yet to be implemented.
               For the time being, we simply fill the r->sig with ones
               to mark that the deserialized number is incorrect. */
              int i;
      ...
      if (image3 | image2 | image1 | image0)
        {
          ...
          r->cl = rvc_nan;
          r->sign = sign;
          r->signalling = ((image3 >> 19) & 1) ^ fmt->qnan_msb_set;
          ...
          lshift_significand (r, r, SIGNIFICAND_BITS - 105);
        }
      else
        {
          /* we have a Inf. */
...
const struct real_format sx_quad_format = 
  {
    encode_sx_quad,
    decode_sx_quad,
    2, 
    1,
    105,
    105 /* significand = 2*52 bits for mantissa + 1 hidden bit */,
    -1021,
    1024,
    127,
    127,
